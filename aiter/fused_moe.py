    ...
    if (kernelName1 and "ck2stages" in kernelName1) or (
        ...
        and (
            ...
            in [
                ...
                dtypes.fp4x2,
                dtypes.fp8,
                dtypes.i8,  # W4A16: int4 weights packed as i8
            ]
        )
    ):
        ...
        # FlyDSL supports: fp8 (W8A8) and int4_bf16 (W4A16)
        _is_flydsl_fp8 = (
...
def flydsl_moe_stage1(
    ...
    **_kwargs,
    ...
    # Infer group_size from w1_scale shape if not explicitly set
    # Per-row scale: [E, N] (2D) or [E*N] (1D)
    # Groupwise scale: [E, N, K//gs] (3D)
    if group_size == -1 and w1_scale is not None and w1_scale.dim() == 3:
        _num_groups_s1 = w1_scale.shape[2]
        if _num_groups_s1 > 1:
            group_size = model_dim // _num_groups_s1
...
def flydsl_moe_stage2(
    ...
    **_kwargs,
    ...
    # Infer group_size from w2_scale shape if not explicitly set
    # Per-row scale: [E, N] (2D) or [E*N] (1D)
    # Groupwise scale: [E, N, K//gs] (3D) where K = inter_dim for stage2
    group_size = -1  # default per-row
    if w2_scale is not None and w2_scale.dim() == 3:
        _num_groups_s2 = w2_scale.shape[2]
        if _num_groups_s2 > 1:
            group_size = inter_dim // _num_groups_s2
    ...
    try:
        ...
        pass
    logger.debug("[flydsl] moe stage2 using mode=%s", str(moe_mode))
    ...
    exe2 = compile_moe_gemm2_ex(
        ...
        # W4A16 support
        in_dtype="int4_bf16" if w2.dtype in (dtypes.i8, torch.int8) else "fp8",
        out_dtype=out_dtype,
        mode=moe_mode,
        group_size=group_size,
    )
    stream_ptr2 = torch.cuda.current_stream().cuda_stream
    exe2(
        out,
        a2_qt_flat,
        w2_flat.view(-1),
